...
API.prototype.getFeeLevels = function(coin, network, cb) {
  ...
  $.checkArgument(
    network || _.includes(['livenet', 'testnet', 'regtest'], network)
  );
    ...
    if (paypro) {
      ...
      PayPro.send(
        ...
        function(err, ack, memo) {
          log.warn(
            'Merchant rejected the payment. Broadcasting it any ways.',
            err
          );
          if (memo) {
...
API.prototype.addAccess = function(opts, cb) {
  ...
  this._doPutRequest('/v1/copayers/' + copayerId + '/', opts, function(
    err,
    res
  ) {
    if (err) return cb(err);
...
API.prototype.editTxNote = function(opts, cb) {
  ...
  self._doPutRequest('/v1/txnotes/' + opts.txid + '/', opts, function(
    err,
    note
  ) {
    if (err) return cb(err);
...
API.prototype.getFiatRate = function(opts, cb) {
  ...
  self._doGetRequest('/v1/fiatrates/' + opts.code + '/' + qs, function(
    err,
    rates
  ) {
    if (err) return cb(err);
