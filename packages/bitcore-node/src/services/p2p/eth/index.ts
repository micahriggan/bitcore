...
import { BitcoreP2PEth } from './ethereum-p2p';
...
export class EthP2pService {
  ...
  constructor(params) {
    ...
    this.adapter = new EthereumAdapter();
    this.syncing = true;
  ...
  convertTransaction(tx: Ethereum.Transaction, block?: Bucket<IBlock>) {
    return this.adapter.convertTx({ chain: this.chain, network: this.network, tx, block });
  }

  async processBlock(block): Promise<any> {
    const convertedBlock = this.adapter.convertBlock({ chain: this.chain, network: this.network, block });
    if (block.transactions.length > 1) {
      console.log('Block has ', block.transactions.length, 'transactions');
    }
    const convertedTransactions = block.transactions.map(t => this.convertTransaction(t, convertedBlock));
